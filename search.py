...
def tree_search[T](problem: Problem[T], strategy: SearchStrategy[T]) -> tuple[TreeSearchNode[T] | None, int]:
    expanded = 0
...
def tree_search_early_goal_test[T](problem: Problem[T], strategy: SearchStrategy[T]) -> tuple[TreeSearchNode[T] | None, int]:
    expanded = 0
    node = _create_node(problem.start_state, 0, 0, None)
    if problem.is_goal(node.state):
        return node, expanded
...
def _expand[T](node: TreeSearchNode[T],
